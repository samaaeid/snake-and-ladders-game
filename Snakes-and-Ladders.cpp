...
//This function prints the grid of Snakes and Ladders as t he game progresses
void print_grid() {
...
return (player_position[player]==100);
}
...
void move_player(int player, int moves) {
    ...
    convert_position_to_indices(player_position[player],i,j);
    grid[i][j]='.';
    }
    player_position[player]+=moves;
    if(player_position[player]>=100){
      convert_position_to_indices(100,i,j);
    }
    else {convert_position_to_indices(player_position[player],i,j);}
