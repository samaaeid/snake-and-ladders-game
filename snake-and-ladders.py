...
def convert_indices_to_position(i, j):   
    if (j==0 and i%2==0)or(j==N-1 and i%2):
        x=0
        y=(10-i)*10 
        p=y+x
...
def move_player(player, moves):
    if player_position[player]:
        i,j=convert_position_to_indices(player_position[player])
        grid[i][j]='.'
    player_position[player]+=moves

    if player_position[player]>=100:
       i,j=convert_position_to_indices(100)
...
def grid_clear():
    global grid,ladders,snakes,player_position
    grid = [[ '.' for x in range(N)]for y in range(M)]
    snakes=[(0,0)]*n_components
    ladders=[(0,0)]*n_components
    player_position=[0,0]
